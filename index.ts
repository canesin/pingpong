...
let ENV = EnvironmentConfiguration.sandbox
let MARKET = process.argv[2].toLowerCase()

switch (process.argv[3]) {
    case 'prod':
        APIKEY = '../pingpong_prod_key.json'
        ENV = EnvironmentConfiguration.production
        break;
    default:
        break;
}

const config = require('../config.json')[MARKET.toUpperCase()] as Config
...
async function getassetprice(): Promise<Spread> {
    try {
        const response = await fetch(SERVICEURL + MARKET.toUpperCase());
...
async function getInitialOrderBook(client: Client): Promise<BotOrderBook> {
...
function updateBotOrderBook(currentOB: BotOrderBook, obUpdate: OrderBook): BotOrderBook {
...
function edgeTrade(trades: Trade[], mkt: MarketSettings): void {
    ...
    trades.forEach(trade => {
        ...
        const sellprice = (
            parseFloat(trade.limitPrice.amount) + mkt.tick
        ...
        player
            ...
            createCurrencyPrice(sellprice, mkt.bUnit, mkt.aUnit),
            MARKET
            )
            .then(placedorder => {
            ...
            if (placedorder.status == OrderStatus.CANCELLED) {
                const sellPrice = (
...
function computebuyprice(spread: Spread, ob: BotOrderBook, mkt: MarketSettings) {
...
function configureConnection(connection: NashSocketEvents, orderbook: BotOrderBook, mkt: MarketSettings) {
...
const run = async () => {
    ...
    for (let iteration = 0; iteration < +Infinity; iteration++) {
        ...
        if (buyingCounter === 0 && edgeCounter <= 1) {
            ...
            try {
                const placedorder = await player.placeLimitOrder(
                    false,
                    createCurrencyAmount(buysize, mkt.aUnit),
                    OrderBuyOrSell.BUY,
                    OrderCancellationPolicy.GOOD_TIL_CANCELLED,
                    createCurrencyPrice(buyprice, mkt.bUnit, mkt.aUnit),
                    MARKET
                )
                .then((placedorder: { status: OrderStatus }) => {
                    if (placedorder.status !== OrderStatus.CANCELLED) {
                        ++buyingCounter;
                        console.log(`++buyingCounter ${buyingCounter}`);
                    }
